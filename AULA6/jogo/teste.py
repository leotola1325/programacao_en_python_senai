...
while True:
      for eventos in pygame.event.get():
          ...
          if eventos.type == pygame.KEYDOWN:  # ====================== dar funções a cada tecla do teclado 
               if eventos.key  == pygame.K_RIGHT:    # ----------------------------- do lado do "k_" programamos qual tecla tera a função
                  quadrado.move_ip([10,0])       # -------------- move 10 px para frente e 0 px para cima
               if eventos.key == pygame.K_LEFT:
                    quadrado.move_ip([-10,0])     # ----------------- move 10px para atras e 0 para cima 

               if eventos.key == pygame.K_UP:
                    quadrado.move_ip([0,-10])    # -------------- move  10px para baixo 

               if eventos.key == pygame.K_DOWN:
                    quadrado.move_ip([0,10])           # ---------- move 10px para cima
          tela.fill('red')
