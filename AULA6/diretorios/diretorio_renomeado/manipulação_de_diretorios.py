...
with open('arquivo.txt', 'w') as arq:
    arq.write("Olis eu criei pelo Python yupi.")
        # |
        # |---------------> criar (write) 

with open('arquivo.txt', 'r') as arq:     # ------------------------ > ele le o arquivo
...
print("Arquivos no diretório renomeado:")
...
shutil.copy('arquivo.txt', 'diretorio_renomeado/arquivo_copiado.txt')  # (shutil.copy) ==  Copiar arquivo
print("Arquivo copiado para diretorio_renomeado")


if os.path.exists('diretorio_renomeado'):
    if not os.path.exists('diretorio_copia'):
        shutil.copytree('diretorio_renomeado', 'diretorio_copia')     # ===================== copiar todo o repositorio 
        print("Diretório copiado para: diretorio_copia")



# 6. Remover arquivos/diretórios
if os.path.exists('arquivo.txt'):   # --------------------------- remover o arquivo caso ele exista 
    os.remove('arquivo.txt')
    print("arquivo.txt removido.")
# Remove diretório recursivamente
if os.path.exists('diretorio_copia'):    #remoção da copia
